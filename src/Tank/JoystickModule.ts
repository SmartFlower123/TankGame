...
class JoystickModule extends egret.EventDispatcher {
    ...
    private OnTouchMove(evt: egret.TouchEvent): void {
        if (this.touchId != -1 && evt.touchPointID == this.touchId) {
            ...
            //============================计算方向
            if (degree <= -45 && degree >= -145) {
                this.m_runDirction = MoveDirection.UP;
            }
            else if (degree < -145 || degree > 145) {
                this.m_runDirction = MoveDirection.LEFT;
            }
            else if (degree > -45 && degree < 45) {
                this.m_runDirction = MoveDirection.RIGHT;
            }
            else if (degree > 45 && degree <= 145) {
                this.m_runDirction = MoveDirection.DOWN;
            }
            //  this.dispatchEventWith(JoystickModule.JoystickMoving, false, degree,this.m_runDirction);
            this.dispatchEventWith(JoystickModule.JoystickMoving, false, this.m_runDirction);
        }
    }
}
